...
def init_db() -> None:
    ddl = [
        ...
        CREATE TABLE IF NOT EXISTS page_categories (
            ...
            PRIMARY KEY (page_title, category_name)
        )
        """,
        """
        ...
        for r in records:
            ...
            if error is None:
                for target in links:
                    cur.execute(
                        """
                        INSERT OR IGNORE INTO page_links (source_title, target_title)
                        VALUES (?, ?)
                        """,
                        (title, target),
                    )
                ...
                for category in categories:
                    cur.execute(
                        """
                        INSERT OR IGNORE INTO page_categories (page_title, category_name)
                        VALUES (?, ?)
                        """,
                        (title, category),
                    )
...
def load_nodes_layout() -> list[dict]:
    with cursor() as (_, cur):
        cur.execute(
            """
            SELECT page_title, x, y, cluster_id, cluster_color, in_degree, out_degree, node_size
            FROM nodes_layout
            """
        )
        rows = cur.fetchall()
